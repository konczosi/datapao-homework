...
def scrape_awards(movie_id: str) -> int:
    ...
    if response.status_code == 200:
        ...
        print(
            f"Cannot access movie's award webpage. Url: {url}, status code: {response.status_code}"
        )
        sys.exit(1)
...
def scrape_top_movies(movies_count: int = 20) -> pd.DataFrame:
    ...
    if response.status_code == 200:
        soup = BeautifulSoup(response.text, "html.parser")
        raw_movies = soup.find("tbody", class_="lister-list").find_all(
            "tr", limit=movies_count
        )
        movies = []
        ...
        for raw_movie in raw_movies:
            ...
            ratings_count = raw_movie.find("span", attrs={"name": "nv"}).get(
                "data-value"
            )
        ...
        print(
            f"Cannot access IMDB webpage. Url: {url}, status code: {response.status_code}"
        )
        sys.exit(1)
